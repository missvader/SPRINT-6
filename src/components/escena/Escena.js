...
function Escena({ historias }) {
  ...
  return (
    ...
    //con ternario establecemos la condicion para que cuando llegue al principio o al final de las historias se pare 
    <Container>
      <DivButtons>
        <Button onClick={() => setCount(count <= 1 ? count : count - 1)}>
          Anterior
        </Button>
        <Button onClick={() => setCount(count >= 4 ? count : count + 1)}>
          Següent
        </Button>
      </DivButtons>
